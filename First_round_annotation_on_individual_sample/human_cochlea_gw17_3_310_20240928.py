...
adata=sc.read_10x_h5("coch-310_filtered_feature_bc_matrix_gw17.h5")
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    ...
    save="_cochlea-cell_type_gw17_3_FIRST_ROUND_SCREENING_1.pdf"
)
...
'Mast Cells':["PRSS34"],
 ...
 'Surrounding structures':["OSR2","BMP6","SLC7A11","CHRDL1","COL1A1","ALDH1A2"],
    ...
    'Inner_border-phalangeal/Hensen_cells':['S100A1','SLC1A3'],
     ...
     'TypeI_SGN' :['SNAP25','TUBB3',   'NEFL','PVALB','CALB2','LYPD1','RUNX1'],
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw17_3_FIRST_ROUND_SCREENING_2.pdf"
)
...
exclude_clusters = ['17','22','24','26','30']
...
#subdivide cluster 27 
sc.tl.leiden(adata_rm, resolution=0.8, restrict_to=('leiden', ['27']))
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw17_3_FIRST_ROUND_SCREENING_3.pdf"
)
...
sc.tl.leiden(adata_rm, resolution=0.8, restrict_to=('leiden', ['28']))
sc.pl.umap(adata_rm, color='leiden_R',legend_loc='on data')
...
#subdivide cluster 25 
sc.tl.leiden(adata_rm, resolution=0.8, restrict_to=('leiden', ['25']))
sc.pl.umap(adata_rm, color='leiden_R',legend_loc='on data')
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw17_3_FIRST_ROUND_SCREENING_4.pdf"
)
...
#subdivide cluster 25,0 
sc.tl.leiden(adata_rm, resolution=0.8, restrict_to=('leiden', ['25,0']))
sc.pl.umap(adata_rm, color='leiden_R',legend_loc='on data')
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw17_3_FIRST_ROUND_SCREENING_5.pdf"
)
...
#subdivide cluster 10 
sc.tl.leiden(adata_rm, resolution=0.8, restrict_to=('leiden', ['10']))
sc.pl.umap(adata_rm, color='leiden_R',legend_loc='on data')
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw17_3_FIRST_ROUND_SCREENING_6.pdf"
)
 ...
 '21': 'Mesenchymal',
  ...
  '7': 'GCs',
   ...
   '14': 'CCs',
    '35': 'Erythrocytes',
    '28,2': 'pericytes',
    '28,1': 'Smooth_Muscle_Cells',
     '19': 'Intermediate_stria',
   '8': 'Fibrocytes',
    '15': 'Fibrocytes',
 '6': 'SS',
     '4': 'Osteocytes/Osteoblasts',
 '20': 'Tympanic_border_cells', 
 '25,0,0': 'Reissner_membrane',   
 '25,0,1': 'Reissner_membrane',   
 '25,0,2': 'Reissner_membrane',   
     '25,1': 'Basal_stria',
'25,4': 'Marginal_stria',
    '27,0': 'Inner_border-phalangeal/Hensen_cells',
 '27,1': 'Deiters_cells', 
 '27,2': 'Pillar_cells',
    '27,3': 'Pillar_cells',
      '23': 'Interdental_cells',
    '31': 'Claudius/Inner-Outer_sulcus_cells',
...
sc.pp.neighbors(adata_rm, n_neighbors=40, n_pcs=60)
...
sc.pl.correlation_matrix(adata_rm, 'cell_type',
                         save="cell-type-correlation_HUMAN_GW17_3_310.pdf")
...
sc.pl.umap(adata_rm,color='cell_type',legend_loc='on data',frameon=False, legend_fontsize=5, legend_fontoutline=False,
           title="Human_fetal_cochlea_GW17_3_310 n=7,148 nuclei",
          save="_HUMAN_GW17_3_310_CELL_TYPE_ANNO.pdf")
sc.set_figure_params(figsize=(4,4),frameon=False,dpi=150,dpi_save=600)
sc.pl.umap(adata_rm,color='cell_type',frameon=False, legend_fontsize=8, legend_fontoutline=False,
           title="Human_fetal_cochlea_GW17_3_310 n=7,148 nuclei",
          save="_HUMAN_GW17_3_310_CELL_TYPE_ANNO_2.pdf")


# In[83]:


adata_rm.write("human_gw17_3_310_raw.h5ad")
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
     save="cochlea-cell_type_refined_human_gw17_3_310-order.pdf"
)
...
sc.pl.violin(adata_rm, ['n_genes_by_counts'], 
             save="cochlea-cell_type_refined_human_gw17_3_310_gene_detection.pdf",
...
sc.pl.violin(adata_rm, ['total_counts'], 
             save="cochlea-cell_type_refined_human_gw17_3_310_total_counts.pdf",
             groupby='cell_type',rotation=90)
...
sc.pl.violin(adata_rm, ['pct_counts_mt'], 
             save="cochlea-cell_type_refined_human_gw17_3_310_mt_pct.pdf",
             groupby='cell_type',rotation=90)
...
adata_rm=sc.read("human_gw17_3_310_raw.h5ad")
adata_rm
...
adata_new.write("human_gw17_3_310_rawcounts.h5ad")
...
adata_gw17_coe=adata_rm[adata_rm.obs['cell_type'].isin(['Claudius/Inner-Outer_sulcus_cells','Inner_border-phalangeal/Hensen_cells',
                                                   'Deiters_cells','Interdental_cells','Pillar_cells',
                                                           ]
...
adata_gw17_coe.write("human_gw17_3_310_raw_coe.h5ad")
