...
adata=sc.read_10x_h5("coch-305_filtered_feature_bc_matrix_gw20.h5")
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    ...
    save="_cochlea-cell_type_gw20_FIRST_ROUND_SCREENING_1.pdf"
)
...
'Mast Cells':["PRSS34"],
 ...
 'Surrounding structures':["OSR2","BMP6","SLC7A11","CHRDL1","COL1A1","ALDH1A2"],
    ...
    'Inner_border-phalangeal/Hensen_cells':['S100A1','SLC1A3'],
     ...
     'TypeI_SGN' :['SNAP25','NEFL','TUBB3',],
    'HC':['TMC1','MYO7A','ATOH1','CALB1',],
    'OHC':['IKZF2','INSM1','SLC26A5',],
       'IHC':['OTOF','SLC17A8','TBX2'],
    'Epithelium':[  "EPCAM",],
     'Cochlear hair cells':[ "ATOH1","MYO7A","TMC1","PCP4",],
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw20_FIRST_ROUND_SCREENING_2.pdf"
)
...
exclude_clusters = ['0','2','3','9','11','13','16','17','19','22','23','26','31','33',]
...
#subdivide cluster 28 
sc.tl.leiden(adata_rm, resolution=2, restrict_to=('leiden', ['28']))
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw20_FIRST_ROUND_SCREENING_3.pdf"
)
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw20_FIRST_ROUND_SCREENING_4.pdf"
)
...
#subdivide cluster 20 
sc.tl.leiden(adata_rm, resolution=1, restrict_to=('leiden', ['20']))
sc.pl.umap(adata_rm, color='leiden_R',legend_loc='on data')
...
#subdivide cluster 21 
sc.tl.leiden(adata_rm, resolution=0.8, restrict_to=('leiden', ['21']))
sc.pl.umap(adata_rm, color='leiden_R',legend_loc='on data')
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw20_FIRST_ROUND_SCREENING_5.pdf"
)
...
#subdivide cluster 29 
sc.tl.leiden(adata_rm, resolution=1, restrict_to=('leiden', ['29']))
sc.pl.umap(adata_rm, color='leiden_R',legend_loc='on data')
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw20_FIRST_ROUND_SCREENING_6.pdf"
)
...
'28,4': 'Marginal_stria',
 '28,1': 'Basal_stria',
'28,0': 'Spindle/Root_cells',
 '20,3': 'Inner_border-phalangeal/Hensen_cells',
 '20,1': 'Inner_border-phalangeal/Hensen_cells',
  '20,4': 'Deiters_cells', 
'20,0': 'Deiters_cells',
     '20,2': 'Pillar_cells',
 '21,2': 'Interdental_cells',
'21,1': 'Interdental_cells',
'21,0': 'Interdental_cells',
'21,4': 'Claudius/Inner-Outer_sulcus_cells',
'21,3': 'Claudius/Inner-Outer_sulcus_cells',
 '29,1': 'pericytes',
    '29,0': 'Smooth_Muscle_Cells',
  '29,2': 'Smooth_Muscle_Cells',
      '29,3': 'Smooth_Muscle_Cells',

    '4': 'Tympanic_border_cells',   
     '6': 'Tympanic_border_cells',  
     '8': 'Tympanic_border_cells',  
     '15': 'Tympanic_border_cells',  
     '27': 'Tympanic_border_cells',  
...
sc.pl.correlation_matrix(adata_rm, 'cell_type',
                         save="cell-type-correlation_HUMAN_GW20_305.pdf")
...
sc.pl.umap(adata_rm,color='cell_type',legend_loc='on data',frameon=False, legend_fontsize=5, legend_fontoutline=False,
           title="Human_fetal_cochlea_GW20_305 n=3,833 nuclei",
          save="_HUMAN_GW20_305_CELL_TYPE_ANNO.pdf")
sc.set_figure_params(figsize=(4,4),frameon=False,dpi=150,dpi_save=600)
sc.pl.umap(adata_rm,color='cell_type',frameon=False, legend_fontsize=8, legend_fontoutline=False,
           title="Human_fetal_cochlea_GW20_305 n=3,833 nuclei",
          save="_HUMAN_GW20_305_CELL_TYPE_ANNO_2.pdf")


# In[82]:


adata_rm.write("human_gw20_305_raw.h5ad")
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
     save="cochlea-cell_type_refined_human_gw20_305-order.pdf"
)
...
sc.pl.violin(adata_rm, ['n_genes_by_counts'], 
             save="cochlea-cell_type_refined_human_gw20_305_gene_detection.pdf",
...
sc.pl.violin(adata_rm, ['total_counts'], 
             save="cochlea-cell_type_refined_human_gw20_305_total_counts.pdf",
             groupby='cell_type',rotation=90)
...
sc.pl.violin(adata_rm, ['pct_counts_mt'], 
             save="cochlea-cell_type_refined_human_gw20_305_mt_pct.pdf",
             groupby='cell_type',rotation=90)
...
adata_rm=sc.read("human_gw20_305_raw.h5ad")
adata_rm
...
adata_new.write("human_gw20_305_rawcounts.h5ad")
...
adata_gw20_coe=adata_rm[adata_rm.obs['cell_type'].isin(['Claudius/Inner-Outer_sulcus_cells','Interdental_cells',
...
adata_gw20_coe.write("human_gw20_305_raw_coe.h5ad")
