...
adata = sc.read_10x_mtx('./filtered_feature_bc_matrix_25W')
print(adata)
...
adata_filtered = adata[adata.obs.n_genes_by_counts < 7500, :]
adata_filtered = adata_filtered[adata_filtered.obs.total_counts > 500, :]
adata_filtered = adata_filtered[adata_filtered.obs.total_counts < 25000, :]
adata_filtered = adata_filtered[adata_filtered.obs.pct_counts_mt < 4, :]
adata_filtered
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    ...
    save="_cochlea-cell_type_gw25_FIRST_ROUND_SCREENING_1.pdf"
)
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw25_FIRST_ROUND_SCREENING_2.pdf"
)
...
exclude_clusters = ['0','5','6','7','9','10','15','18','19','25','27']
...
#subdivide cluster 20 for smc pericytes
sc.tl.leiden(adata_rm, resolution=0.8, restrict_to=('leiden', ['20']))
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw25_FIRST_ROUND_SCREENING_3.pdf"
)
...
# 1st remove ambigous clusrer then subdivide particular clusters
exclude_clusters = ['20,1','20,2','20,4']
...
#subdivide cluster 13 for sc
sc.tl.leiden(adata_rm, resolution=0.8, restrict_to=('leiden', ['13']))
sc.pl.umap(adata_rm, color='leiden_R',legend_loc='on data')
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw25_FIRST_ROUND_SCREENING_4.pdf"
)
...
#subdivide cluster 32 for hc bs
sc.tl.leiden(adata_rm, resolution=2, restrict_to=('leiden', ['32']))
sc.pl.umap(adata_rm, color='leiden_R',legend_loc='on data')
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
    save="_cochlea-cell_type_gw25_FIRST_ROUND_SCREENING_5.pdf"
)
...
# 1st remove ambigous clusrer then subdivide particular clusters
exclude_clusters = ['32,0','32,2','32,3','32,5','32,7','32,9','32,10','32,11','32,12','32,13','32,14']
adata_rm= adata_rm[~adata_rm.obs['leiden'].isin(exclude_clusters), :]
adata_rm
...
'21': 'Endothelial',
 ...
 '11': 'Intermediate_stria',
    '23': 'Intermediate_stria', 
'2': 'Fibrocytes',
   '14': 'Osteoblasts',
'3': 'Osteocytes',
 '1': 'Tympanic_border_cells', 
'29': 'Tympanic_border_cells', 
 '26': 'SS',  
'32,1': 'Basal_stria',
  '22': 'Deiters_cells', 
 '31': 'Marginal_stria',
   
  '13,5': 'Inner_border-phalangeal/Hensen_cells',

  '13,2': 'Pillar_cells',
     '13,4': 'Interdental_cells',
    
    '13,0': 'Claudius/Inner-Outer_sulcus_cells',
  '13,1': 'Claudius/Inner-Outer_sulcus_cells',
      '13,3': 'Claudius/Inner-Outer_sulcus_cells',
 '32,6': 'HCs',
'32,4': 'HCs',
'32,8': 'HCs',
...
sc.pl.correlation_matrix(adata_rm, 'cell_type',
                         save="cell-type-correlation_HUMAN_GW25.pdf")
...
sc.pl.umap(adata_rm,color='cell_type',legend_loc='on data',frameon=False, legend_fontsize=5, legend_fontoutline=False,
           title="Human_fetal_cochlea_GW25 n=13,936 nuclei",
          save="_HUMAN_GW25_CELL_TYPE_ANNO.pdf")
sc.set_figure_params(figsize=(4,4),frameon=False,dpi=150,dpi_save=600)
sc.pl.umap(adata_rm,color='cell_type',frameon=False, legend_fontsize=8, legend_fontoutline=False,
           title="Human_fetal_cochlea_GW25 n=13,936 nuclei",
          save="_HUMAN_GW25_CELL_TYPE_ANNO_2.pdf")


# In[68]:


adata_rm.write("human_gw25_raw.h5ad")
...
sc.pl.dotplot(
   ...
   dendrogram=True,
    standard_scale="var",  # standard scale: normalize each gene to range from 0 to 1
     save="cochlea-cell_type_refined_human_gw25-order.pdf"
)
...
sc.pl.violin(adata_rm, ['n_genes_by_counts'], 
             save="cochlea-cell_type_refined_human_gw25_gene_detection.pdf",
...
sc.pl.violin(adata_rm, ['total_counts'], 
             save="cochlea-cell_type_refined_human_gw25_total_counts.pdf",
             groupby='cell_type',rotation=90)
...
sc.pl.violin(adata_rm, ['pct_counts_mt'], 
             save="cochlea-cell_type_refined_human_gw25_mt_pct.pdf",
             groupby='cell_type',rotation=90)
...
adata_rm=sc.read("human_gw25_raw.h5ad")
adata_rm
...
adata_new.write("human_gw25_rawcounts.h5ad")
...
adata_gw25_coe=adata_rm[adata_rm.obs['cell_type'].isin(['Claudius/Inner-Outer_sulcus_cells','Interdental_cells',
...
adata_gw25_coe.write("human_gw25_raw_coe.h5ad")
